...
class Test{
    ...
    excel(){
        ...
        for (let i = 0; i < this.dfs.length-1; i++) {
            data.push([this.dfs[i].market,this.dfs[i].candle_date_time_kst,this.dfs[i].opening_price,
            this.dfs[i].high_price,this.dfs[i].low_price,this.dfs[i].trade_price])
        }
    ...
    async play(){
        ...
        this.df = await upbit.market_minute(this.market,this.tick_kind,200,'2021-04-14T23:00:00Z');
        ...
        //    }
            if ( this.dfs[i].opening_price * 1.01 < this.dfs[i].high_price) {
                ...
                let rate = this.dfs[i].opening_price * 1.02 < this.dfs[i].high_price ?
                    1.01 - 0.001: (sell_price / buy_price) - 0.001;
                    this.acc_ror *= rate;
        }
    }
