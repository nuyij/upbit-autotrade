...
class Test{
    ...
    async play(){
        ...
        console.log(this.dfs.length);
        console.log(data.slowK.length);
        console.log(data.slowD.length);
        ...
        //excel
        {
            ...
            for (let i = 0; i < slowD.length-2; i++) {
                const isBuy = (slowK[i+2]<slowD[i+2])&&(slowK[i+1]>slowD[i+1]) ? true : false;
            ...
            console.log(num);
            const input = [['market','time','open','high','low','trade','buy','sell']];
            for (let i = buy.length-1; i >= 0; i--) {
                input.push([this.dfs[i].market,this.dfs[i].candle_date_time_kst,this.dfs[i].opening_price,
                        this.dfs[i].high_price,this.dfs[i].low_price,this.dfs[i].trade_price,buy[i],sell[i]])
            }
            const workbook = xlsx.utils.book_new();
