    ...
    async bot() {
        while (true) {
            if (!this.hold) {
                ...
                const ri = items.filter((item => { return (item.scr > 0.05) && (item.cd.startsWith("KRW")) && (!item.cd.startsWith("KRW-BTT")) }))
                ...
                for (let i = 0; i < this.set.length; i++) {
                    if(!this.ready){
                        ...
                        if (data) {
                            ...
                            this.condition = (this.open * 1.015 <= high)
            ...
            if (data) {
                const macd = upbit.get_macd(data)
                const stochastic = upbit.get_stochastic(data)
                this.open = data[0].opening_price;
                const low = data[0].low_price;
                const high = data[0].high_price;
                this.trade = data[0].trade_price;
                ...
                this.condition = (this.open * 1.05 <= high)&&(stochastic.slowK[0]>stochastic.slowD[0])
                if (this.condition) this.ready = true;
