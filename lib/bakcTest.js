...
class Test{
    ...
    ma_condition(){
        for (let i = 0; i < this.dfs.length; i++) {
            const ma5 = this.ma(this.dfs,i,5)
            const ma20 = this.ma(this.dfs,i,20)
            const ma100 = this.ma(this.dfs,i,100)
            const condition1 = ma5 > ma20;
            const condition2 = ma20 > ma100;

            if (condition1 && condition2 && this.dfs[i].opening_price * 1.01 < this.dfs[i].high_price) {
