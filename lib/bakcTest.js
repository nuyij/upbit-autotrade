...
class Test{
    ...
    async play(){
        ...
        // 1d = 1440min = 1.5
        // '2022-05-10T08:50:00'
        let df = await upbit.market_minute(this.market,5,200,'2022-05-10T08:50:00');
        dfs.push(df.data);
        console.log(df);
        console.log(df.data[df.data.length-1].candle_date_time_kst);
        for (let i = 0; i < 10; i++) {
            df = await upbit.market_minute(this.market,5,200,df.data[df.data.length-1].candle_date_time_kst)
            dfs.push(df.data);
            await sleep(200)
        }
