...
async function get_ma(market,tick_kind,range){
    const ma_list = [];
    await market_minute(market,tick_kind,50)
    .then((res)=>{
        const candles = res.data;
        let tot = 0;
        for (let i = 0; i < range; i++) {
            for (let j = i; j <range+i; j++) {
                const price = res.data[j].trade_price;
                tot += price;
            }
            const ma = tot/range;
            const data = {'trade_price':candles[i].trade_price, 'ma':ma}
            ma_list.push(data)
        }
    })
    return ma_list;
...
async function get_ema(market,tick_kind,range){
    const ema_list = [];
    const ma_list = await get_ma(market,tick_kind,50);
    console.log(ma_list);
    // .then((res)=>{
    //     for (let i = 0; i < range; i++) {
    //         let ema = 0;
    //         for (let j = i; j < range+i; j++) {
    //             const ma = res[i].ma;
    //             let ema_ = ema_list[i-1];
    //             const price = res[i].trade_price;
    //             const mul = 2/(1+range);

    //             if(i=0){ema_ = res[i+1].ma} 
    //             ema += (price*mul)+(ema_*(1-mul));
    //         }
    //         ema_list.push(ema/range);
    //     }
    // })
    return ema_list
}
...
async function get_macd(market,tick_kind){
    ...
    const line26 = await get_ema(market,tick_kind,26);
    const line12 = await get_ema(market,tick_kind,12);
    let macd_list = [];
    let signal;
    let oscillator;
    ...
    let tot = 0;
        for (let i = 0; i < 9; i++) {
           const macd = line12[i]-line26[i]
           macd_list.push(macd);
            tot += macd;
        }
        signal = tot/9;
        oscillator = macd - signal;
    return {"macd":macd_list[0],"signal":signal,'oscillator':oscillator};
