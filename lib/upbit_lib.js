...
function ema(maData,tradeData,range){
  ...
  for (let i = 1; i < ma.length; i++) {
    const ema_previous = i == 1 ? ma[0] : ema_list[i-2];
    const mul= 2 / (range + 1);
...
function get_stochastic(data){
  // for (let k = 0; k < data.length-range-m-1; k++) {
  //   slowK_list = [];
  //   for (let j = k+0; j < m+k; j++) {
  //     low = 100000000;
  //     high = 0;
  //     slowK = 0;
  //     for (let i = j+0; i < j+range; i++) {
  //       low = data[i].trade_price < low ? data[i].trade_price : low;
  //       high = data[i].trade_price > high ? data[i].trade_price : high;
  //     }
    
  //     for (let i = j+0; i < j+n; i++) {
  //       const trade = data[i].trade_price;
  //       const fastK = parseFloat((trade-low)*100/(high-low));
  //       slowK += fastK;
  //     }
  //     slowK = parseFloat(slowK/n);
  //     slowK_list.push(slowK)
  //     slowD += slowK;
  //   }
  //   slowD = parseFloat(slowD/m);
  //   list.push({slowK:slowK_list[0].toFixed(2),slowD:slowD.toFixed(2)})
  // }
  const range = 20;
  const n = 10;
  const m = 10;
  ...
  slowK_list = ema(ma(fastK_list,10),fastK_list,10);
  slowD = ema(ma(slowK_list,10),slowK_list,10);
  for (let i = 0; i < n; i++) {
    slowK.push(slowK_list[i])
  }
