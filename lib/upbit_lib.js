...
async function market_minute(market, unit, count) {
    ...
    this.candles_result = result;
    return result
}

// 일 캔들
async function market_day(market, count) {
    //to: yyyy-MM-dd'T'HH:mm:ssXXX
...
function get_bb(){
    ...
    const range = 20;
        ...
        const candles = this.candles_result;
        ...
        const time = this.candles_result[0].candle_date_time_kst;
        const price = this.candles_result[0].trade_price;
...
function get_ma(range){
    const ma_list = [];
            
            const candles = this.candles_result;
                for (let i = 0; i < range; i++) {
                    let tot = 0;
                    const time = this.candles_result[i].candle_date_time_kst;
                    const price = this.candles_result[i].trade_price;
...
function get_ema(range) {
    let ema_list = [];
    const res = get_ma(100)
            if(res ==null || typeof res == 'undefined'){
                setTimeout(()=>{return get_ema(range)},1000)
            }else{
...
function get_macd(){
    const range = 35;
    let line26 = get_ema(26);
    let line12 = get_ema(12);
...
// Upbit.prototype.candles_result = candles_result
